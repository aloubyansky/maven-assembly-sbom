 ...
 *
 * <p>
 * Callers register components via {@link #addMavenArtifact},
 * {@link #addNestedMavenArtifact}, and {@link #addFile}, optionally
 * supply a dependency graph with {@link #setDependencyGraph} and
 * {@link #addExplicitDependency}, then obtain the finished model with
 * {@link #build()}.
 * </p>
 *
 * <p>
 * Top-level Maven artifacts registered via {@link #addMavenArtifact}
 * are deduplicated: only the first registration creates a component,
 * subsequent calls add an additional {@link Occurrence} to the existing
 * component's evidence. Nested registrations ({@link #addNestedMavenArtifact},
 * {@link #addNestedArtifactUnderFile}) always create independent component
 * instances so that each nesting location carries only its own occurrence.
 * </p>
 */
public class BomBuilder {
    ...
    public void addMavenArtifact(ArtifactCoords coords, String archivePath,
        ...
        Component comp = newMavenComponent(coords, archivePath, hash, licenses);
     ...
     *
     * <p>
     * Each call creates an independent {@link Component} instance with its
     * own occurrence evidence, even when an artifact with identical
     * coordinates has already been registered at top level or under another
     * parent. This ensures that a component appearing at multiple locations
     * (e.g. both {@code lib/} and inside a WAR) carries only the occurrence
    ...
    /**
     * Creates a fully initialized Maven component with hash, evidence,
     * and optional licenses.
     */
    private Component newMavenComponent(ArtifactCoords coords, String archivePath,
            String hash, LicenseChoice licenses) {
    ...
    private void buildDependencyTree(Bom bom, String mainBomRef) {
        ...
        addLeafEntriesForMissingRefs(bom);
     ...
     */
    private void addLeafEntriesForMissingRefs(Bom bom) {
        ...
        for (String ref : bomRefById.values()) {
            if (existingRefs.add(ref)) {
                bom.addDependency(new Dependency(ref));
            }
        }
    }
